...
def adversarial_training_step(encoder, decoder, cover, message, *, epsilon=0.03, adv_steps=5, image_loss_fn, message_loss_fn, lambda_adv=0.3):
    ...
    return {"image_loss": float(img_loss.item()), "adv_message_loss": float(adv_msg_loss.item()), "total_loss": float((img_loss + lambda_adv * adv_msg_loss).item())}
